...
public class CT005Steps extends ValidationUtil {

  private final TeamResource teamResource;

  private final Logger logger = LoggerFactory.getLogger(CT005Steps.class);
  ...
  public CT005Steps(TeamResource teamResource) {
    this.teamResource = teamResource;
  }

  @Given("I perform a POST request for {string} informing a Pokemon that not exists in field thirdPokemon")
  public void IPerformAPostRequestForTeamInformingAPokemonThatNotExistsInFieldThirdPokemon(String path) {
    ...
    requestBody.put("thirdPokemon", "Teste");
  ...
  @Then("Verify if returning the message for pokemon that not exists in field first thirdPokemon {string} and response code {string}")
  public void VerifyIfReturningTheMessageForPokemonThatNotExistInFieldThirdPokemon(String statusDescription, String statusCode) {
    String pathBody = "[0]";
