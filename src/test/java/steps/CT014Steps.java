...
public class CT014Steps extends ValidationUtil {

  private final TeamResource teamResource;

  private final Logger logger = LoggerFactory.getLogger(CT014Steps.class);
  ...
  public CT014Steps(TeamResource teamResource) {
    this.teamResource = teamResource;
  }

  @Given("I perform a POST request for {string} not informing the field lastPokemon")
  public void IPerformAPostRequestForTeamNotInformingTheFieldLastPokemon(String path) {
  ...
  @Then(
      "Verify if returning the message not informing the field lastPokemon {string} and response code {string}")
  public void VerifyIfReturningTheMessageNotInformingTheFieldLastPokemon(
