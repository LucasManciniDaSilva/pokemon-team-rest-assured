...
public class CT007Steps extends ValidationUtil {

  private final TeamResource teamResource;

  private final Logger logger = LoggerFactory.getLogger(CT007Steps.class);
  ...
  public CT007Steps(TeamResource teamResource) {
    this.teamResource = teamResource;
  }

  @Given("I perform a POST request for {string} informing a Pokemon that not exists in field fifthPokemon")
  public void IPerformAPostRequestForTeamInformingAPokemonThatNotExistsInFieldFifthPokemon(String path) {
    ...
    requestBody.put("fifthPokemon", "Teste");
  ...
  @Then("Verify if returning the message for pokemon that not exists in field fifthPokemon {string} and response code {string}")
  public void VerifyIfReturningTheMessageForPokemonThatNotExistInFieldFifthPokemon(String statusDescription, String statusCode) {
    String pathBody = "[0]";
